 ...
 *     Variant Mean   — mean across all 9 passive loadouts (ARCHETYPE strength)
 ...
 * The first is one lucky passive combination; the second is a real archetype.
 * Sort by Variant Mean when you want archetype strength, by Best when you want
 * "the strongest single build a player could field".
 *
 * ALSO EMITS a class-representation table (top 10/25/50/100 under both sorts).
 ...
 *              <dir>/top_builds_by_archetype.csv    (sorted by Variant Mean)
...
const byArch = [...builds].sort((a, b) => b.vmean - a.vmean);
writeFileSync(join(dir, 'top_builds.csv'), [HEAD.join(','), ...byBest.map(line)].join('\n') + '\n');
writeFileSync(join(dir, 'top_builds_by_archetype.csv'), [HEAD.join(','), ...byArch.map(line)].join('\n') + '\n');
...
const rep = [['Class',
  ...DEPTHS.flatMap((n) => [`Top${n} (best)`, `Top${n} % (best)`]),
  ...DEPTHS.flatMap((n) => [`Top${n} (archetype)`, `Top${n} % (archetype)`]),
  'All','All %','Baseline %'].join(',')];
const cb = Object.fromEntries(DEPTHS.map((n) => [n, count(byBest, n)]));
const ca = Object.fromEntries(DEPTHS.map((n) => [n, count(byArch, n)]));
const all = count(byBest, byBest.length);
for (const cl of CLASSES) {
  rep.push([cl,
    ...DEPTHS.flatMap((n) => [cb[n][cl], (100 * cb[n][cl] / n).toFixed(1)]),
    ...DEPTHS.flatMap((n) => [ca[n][cl], (100 * ca[n][cl] / n).toFixed(1)]),
...
console.log('  top_builds.csv · top_builds_by_archetype.csv · class_representation_topbuilds.csv\n');
console.log('rank  best  vmean  spread  build');
byArch.slice(0, 10).forEach((b, i) =>
...
console.log(`  ${'class'.padEnd(11)}${DEPTHS.map((n) => `top${n}`.padStart(7)).join('')}   (archetype sort)`);
for (const cl of CLASSES) console.log(`  ${cl.padEnd(11)}${DEPTHS.map((n) => String(ca[n][cl]).padStart(7)).join('')}`);

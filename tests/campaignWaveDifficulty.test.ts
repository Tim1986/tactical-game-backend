...
describe('difficulty-scoped waves', () => {
  ...
  it('e6 doses extra waves by tier: medium +1, hard +1, nightmare +2', () => {
    ...
    // [E6-0903] Nightmare dropped its THIRD wisp (owner: "a ton of them have
    // Ring of Frost") — nightmare now = hard's wisp + one more, plus the
    // shared vanguard chaser.
    const easy = pendingWaveCount('easy');
    expect(pendingWaveCount('medium')).toBe(easy + 1);
    expect(pendingWaveCount('hard')).toBe(easy + 1);
    expect(pendingWaveCount('nightmare')).toBe(easy + 2);
  });

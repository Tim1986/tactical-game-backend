...
describe('isInBounds', () => {
  it('accepts valid positions', () => { expect(isInBounds({ x: 0, y: 0 })).toBe(true); expect(isInBounds({ x: 7, y: 7 })).toBe(true); });
...
describe('calculatePushDestination', () => {
  ...
  it('clamps to board edge', () => { const result = calculatePushDestination({ x: 6, y: 0 }, { x: 0, y: 0 }, 5); expect(result.x).toBe(7); });
});

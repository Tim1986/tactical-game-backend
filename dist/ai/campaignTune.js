...
function meanWinRate(slug, encId, diff, games) {
    const perParty = {};
    let total = 0;
    for (const [pname, pslugs] of Object.entries(campaignSim_js_1.REPRESENTATIVE_PARTIES)) {
        const r = (0, campaignSim_js_1.simEncounterCell)(slug, encId, diff, pname, pslugs, { games });
        perParty[pname] = r.winRate;
        total += r.winRate;
    }
    return { mean: total / Object.keys(campaignSim_js_1.REPRESENTATIVE_PARTIES).length, perParty };
}
function main() {
    const args = process.argv.slice(2);
    const slug = args[0];
    if (!slug || !index_js_1.CAMPAIGNS[slug]) {
    ...
    const diffs = getArg('--difficulty') ? [getArg('--difficulty')] : DIFFICULTIES;
    ...
    for (const encId of Object.keys(campaign.encounters)) {
        ...
        for (const diff of diffs) {
            ...
            const original = enc.hpScaleOverride?.[diff] ?? runtime_js_1.CAMPAIGN_HP_SCALE[diff];
            ...
            let bestMean = 0, bestParties = {};
            for (let i = 0; i < iters; i++) {
                const mid = (loS + hiS) / 2;
                if (!enc.hpScaleOverride)
                    enc.hpScaleOverride = {};
                ...
                if (err < bestErr) {
                    ...
                    best = mid;
                    bestMean = mean;
                    bestParties = perParty;
                }
                if (mean > target)
                    loS = mid;
                else
                    hiS = mid; // too easy -> more HP
            }
            ...
            console.log(`${encId.padEnd(4)} ${diff.padEnd(10)} ${original.toFixed(2)} -> ${rounded.toFixed(2)}  `
...
main();
//# sourceMappingURL=campaignTune.js.map
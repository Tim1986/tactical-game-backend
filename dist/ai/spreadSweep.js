...
const BS = 8;
const isCorner = (p) => (p.x === 0 || p.x === BS - 1) && (p.y === 0 || p.y === BS - 1);
const inBounds = (p) => p.x >= 0 && p.x < BS && p.y >= 0 && p.y < BS;
/** Step `pos` `steps` tiles toward (negative: away from) `target`, keeping it legal. */
function shiftToward(pos, target, steps, taken) {
    ...
    for (let i = 1; i <= Math.abs(steps); i++) {
        ...
        if (!inBounds(cand) || isCorner(cand))
            break;
        if (taken.has(`${cand.x},${cand.y}`))
            continue;
        best = cand;
    }
    return best;
...
function main() {
    ...
    const diffs = (getArg('--diffs') ?? 'medium,hard').split(',');
    const campaign = index_js_1.CAMPAIGNS[campSlug];
    ...
    for (const off of [-2, -1, 0, 1, 2, 3]) {
        ...
        enc.enemyPlacement = moved;
        const gap = moved.reduce((s, p) => s + Math.abs(p.x - centroid.x) + Math.abs(p.y - centroid.y), 0) / moved.length;
        for (const diff of diffs) {
            const w = {};
            for (const [pn, ps] of Object.entries(campaignSim_js_1.REPRESENTATIVE_PARTIES)) {
                w[pn] = (0, campaignSim_js_1.simEncounterCell)(campSlug, encId, diff, pn, ps, { games }).winRate;
            }
            ...
            const flags = (below.length ? `floor:${below.join('')}` : '')
                + (mean < lo || mean > hi ? ' oob' : '');
            console.log(`${(off > 0 ? '+' + off : String(off)).padEnd(7)} ${diff.padEnd(8)} ${pct(mean)}  ${pct(w.melee)}  ${pct(w.ranged)}   ${pct(w.balanced)}  ${(spread * 100).toFixed(0).padStart(3)}pts   ${flags}`);
        }
        console.log(`        (mean manhattan gap ${gap.toFixed(1)}; placement ${moved.map((p) => `${p.x},${p.y}`).join(' ')})\n`);
    }
    enc.enemyPlacement = orig;
}
main();
//# sourceMappingURL=spreadSweep.js.map
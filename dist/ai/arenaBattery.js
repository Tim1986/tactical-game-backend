...
const pct = (n, d) => ((100 * n) / Math.max(1, d)).toFixed(0).padStart(3) + '%';
function defaultLoadout(slugs) {
...
function series(compA, compB, custA, custB, games) {
    const b1 = new aiBrain_js_1.OptimalBrain();
    const b2 = new aiBrain_js_1.OptimalBrain();
    const rng = (0, simHarness_js_1.makeRng)(7);
    let aWins = 0;
    let bWins = 0;
    let draws = 0;
    for (let g = 0; g < games; g++) {
        const r = (0, simHarness_js_1.runMatch)(compA, compB, abilityMap, b1, b2, {
            p1Id: 'p1', p2Id: 'p2',
...
function plateSpecials(games) {
    ...
    for (const [slug, compName] of Object.entries(HOME_COMP)) {
        ...
        const options = defaultData_js_1.DEFAULT_UNITS[slug].specialOptions;
        console.log(`${slug} (${compName}): options ${options.join(', ')}`);
        for (let i = 0; i < options.length; i++) {
            for (let j = i + 1; j < options.length; j++) {
                const cA = defaultLoadout(comp);
                cA[slot] = { ...cA[slot], specialSlug: options[i] };
                const cB = defaultLoadout(comp);
                cB[slot] = { ...cB[slot], specialSlug: options[j] };
                ...
                agg[options[i]].w += r.aWins;
                agg[options[i]].t += dec;
                agg[options[j]].w += r.bWins;
                agg[options[j]].t += dec;
                console.log(`  ${options[i].padEnd(12)} vs ${options[j].padEnd(12)} ${pct(r.aWins, dec)} - ${pct(r.bWins, dec)}` +
...
function plateComps(games) {
    ...
    for (let i = 0; i < names.length; i++) {
        for (let j = i + 1; j < names.length; j++) {
            ...
            const dec = r.aWins + r.bWins;
            score[a] = score[a] ?? { w: 0, t: 0 };
            score[b] = score[b] ?? { w: 0, t: 0 };
            score[a].w += r.aWins;
            score[a].t += dec;
            score[b].w += r.bWins;
            score[b].t += dec;
            console.log(`  ${a.padEnd(9)} vs ${b.padEnd(9)} ${pct(r.aWins, dec)} - ${pct(r.bWins, dec)}` +
                (r.draws > 0 ? `  (${r.draws} draws)` : ''));
        }
    }
...
function plateClasses(games) {
    ...
    for (let slot = 0; slot < base.length; slot++) {
        for (const sub of outside) {
            ...
            classScore[sub].w += r.aWins;
            classScore[sub].t += dec;
            classScore[base[slot]] = classScore[base[slot]] ?? { w: 0, t: 0 };
            classScore[base[slot]].w += r.bWins;
            classScore[base[slot]].t += dec;
            console.log(`  ${sub.padEnd(9)} in for ${base[slot].padEnd(8)} ${pct(r.aWins, dec)} - ${pct(r.bWins, dec)}` +
                (r.draws > 0 ? `  (${r.draws} draws)` : ''));
        }
    }
...
if (plate === 'all' || plate === 'specials')
    plateSpecials(games);
if (plate === 'all' || plate === 'comps')
    plateComps(games);
if (plate === 'all' || plate === 'classes')
    plateClasses(games);
//# sourceMappingURL=arenaBattery.js.map
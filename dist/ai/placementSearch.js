...
const map = (0, defaultData_js_1.buildAbilityMap)();
const normalized = new Map(map);
...
const isMelee = (slug) => {
    const abs = defaultData_js_1.DEFAULT_UNITS[slug].abilities.map((a) => map.get(a)).filter(Boolean);
 ...
 */
function candidates(slugs, custs, rng) {
    const out = [];
    const seen = new Set();
    const push = (p) => {
        if (p.length !== slugs.length)
            return;
        const k = key(p);
        if (seen.has(k) || new Set(p.map((q) => `${q.x},${q.y}`)).size !== p.length)
            return;
        seen.add(k);
        out.push(p);
    };
    push((0, placement_js_1.planPlacement)(slugs, normalized, custs));
    const melee = slugs.map(isMelee);
    const pick = (allowed) => allowed[Math.floor(rng() * allowed.length)];
...
const FIELD = fableTeams_js_1.FABLE_TEAMS.map((t) => {
    const c = (0, fableTeams_js_1.fableCustomizations)(t);
    return { name: t.name, slugs: [...t.slugs], custs: c,
        placement: (0, placement_js_1.mirrorPlacement)((0, placement_js_1.planPlacement)([...t.slugs], normalized, c)) };
});
function evaluate(slugs, custs, placement, games) {
    let W = 0, L = 0;
    const per = [];
    for (const foe of FIELD) {
        let w = 0, l = 0;
        for (let g = 0; g < games; g++) {
            const r = (0, simHarness_js_1.runMatch)(slugs, foe.slugs, map, new aiBrain_js_1.OptimalBrain(), new aiBrain_js_1.OptimalBrain(), {
...
console.log(`Field: all ${fableTeams_js_1.FABLE_TEAMS.length} rosters at heuristic placement.\n`);
fableTeams_js_1.FABLE_TEAMS.forEach((t, ti) => {
    const slugs = [...t.slugs];
    const custs = (0, fableTeams_js_1.fableCustomizations)(t);
    ...
    const heur = evaluate(slugs, custs, (0, placement_js_1.planPlacement)(slugs, normalized, custs), N_CONFIRM);
    ...
    results[t.name] = {
        slugs, heuristic: { placement: (0, placement_js_1.planPlacement)(slugs, normalized, custs), ...heur },
        searched: { placement: best.p, ...best.s },
    };
    node_fs_1.default.writeFileSync(OUT, JSON.stringify(results, null, 1));
});
console.log(`\nWritten to ${OUT}`);
//# sourceMappingURL=placementSearch.js.map
...
const aiBrain_1 = require("./aiBrain");
const defaultData_1 = require("./defaultData");
let nextId = 1;
function mkUnit(slug, owner, pos, hp) {
    const def = defaultData_1.DEFAULT_UNITS[slug];
    return {
...
const brain = new aiBrain_1.OptimalBrain();
const map = (0, defaultData_1.buildAbilityMap)();
let failures = 0;
...
// --- Test 1: Rogue holds Kill Shot above threshold, uses it at/below ---
{
    ...
    const usedKillShotEarly = actions.some((a) => a.type === 'USE_ABILITY' && a.abilitySlug === 'assassinate');
    check('Rogue does NOT waste Kill Shot on full-HP target', !usedKillShotEarly, JSON.stringify(actions));
    ...
    const usedKillShotNow = actions.some((a) => a.type === 'USE_ABILITY' && a.abilitySlug === 'assassinate');
    check('Rogue USES Kill Shot on 18-HP target', usedKillShotNow, JSON.stringify(actions));
}
// --- Test 2: Cleric heals a badly wounded adjacent ally ---
{
    ...
    const healed = actions.some((a) => a.type === 'USE_ABILITY' && a.abilitySlug === 'heal');
...
// --- Test 3: Barbarian avoids Whirlwind that would hit more allies than enemies ---
{
    ...
    const actions = brain.selectActions(state, 'p1', map);
    const whirled = actions.some((a) => a.type === 'USE_ABILITY' && a.abilitySlug === 'whirlwind');
    check('Barbarian skips Whirlwind when 2 allies + 1 enemy adjacent', !whirled, JSON.stringify(actions));
}
// --- Test 4: Ranger attacks from range instead of walking into melee ---
{
    ...
    const actions = brain.selectActions(state, 'p1', map);
    const attacked = actions.some((a) => a.type === 'USE_ABILITY' && a.abilitySlug === 'arrow');
...
// --- Test 6: Round 1 with no activeUnitId — brain picks a unit and commits ---
{
    ...
    const actions = brain.selectActions(state, 'p1', map);
    const identifiesUnit = actions.some((act) => act.type !== 'END_TURN' && 'unitInstanceId' in act);
    check('Round 1: plan identifies which unit is committed', identifiesUnit, JSON.stringify(actions));
    check('Round 1: plan ends with END_TURN', actions[actions.length - 1].type === 'END_TURN');
}
...
// --- Test 8: Sorcerer will not Firestorm its own team ---
{
    ...
    const actions = brain.selectActions(state, 'p1', map);
    const storm = actions.find((a) => a.type === 'USE_ABILITY' && a.abilitySlug === 'ffh');
    if (storm && storm.type === 'USE_ABILITY') {
        ...
        check('Firestorm center avoids the fragile ally', Math.max(dx, dy) > 1, JSON.stringify(storm));
    }
    else {
...
// --- Test 9 (Bug 3): enemies block movement — cornered unit cannot escape ---
{
    ...
    const tiles = (0, geometry_1.reachableTiles)(wizard, [wizard, blocker1, blocker2], wizard.movementRange);
    check('Bug 3: cornered unit has ZERO reachable tiles through enemies', tiles.length === 0, `got ${tiles.length} tiles: ${JSON.stringify(tiles)}`);
}
// --- Test 10 (Bug 3): allies can be passed through but not landed on ---
{
    ...
    const tiles = (0, geometry_1.reachableTiles)(fighter, [fighter, ally, enemyBlock], fighter.movementRange);
...
// --- Test 11 (Bug 2): warlock does not waste Fear on a frozen target ---
{
    ...
    const actions = brain.selectActions(state, 'p1', map);
    const usedFear = actions.some((a) => a.type === 'USE_ABILITY' && a.abilitySlug === 'fear');
...
// --- Test 12 (Bug 2): wizard does not re-freeze an already-frozen target ---
{
    ...
    const actions = brain.selectActions(state, 'p1', map);
    const refroze = actions.some((a) => a.type === 'USE_ABILITY' && a.abilitySlug === 'freeze');
...
// --- Test 13 (Bug 1): Whirlwind vetoed when it could kill a high-AC ally ---
{
    ...
    const actions = brain.selectActions(state, 'p1', map);
    const whirled = actions.some((a) => a.type === 'USE_ABILITY' && a.abilitySlug === 'whirlwind');
    check('Bug 1: Whirlwind hard-vetoed when it could kill high-AC ally', !whirled, JSON.stringify(actions));
}
...
// let the harness/server pre-flight append the unit to initiative.order.
{
    ...
    const actions = brain.selectActions(state, 'p1', map);
    check('Bug A: frozen/dead forced-commit returns bare END_TURN', actions.length === 1 && actions[0].type === 'END_TURN', JSON.stringify(actions));
    ...
    check('Bug A: dead-only forced-commit returns bare END_TURN', actions2.length === 1 && actions2[0].type === 'END_TURN', JSON.stringify(actions2));
}
// ===========================================================================
...
// ~2 full turns of the fighter's output, which must now outscore 12 damage.
{
    ...
    const actions = brain.selectActions(state, 'p1', map);
    const fearOnMelee = actions.some((a) => a.type === 'USE_ABILITY' &&
        a.abilitySlug === 'fear' &&
        a.target.x === meleeEnemy.position.x &&
        a.target.y === meleeEnemy.position.y);
    check('R3: Fear (not Demon Blast) peels the adjacent melee fighter', fearOnMelee, JSON.stringify(actions));
}
...
// Arcane Bolt instead.
{
    ...
    const actions = brain.selectActions(state, 'p1', map);
    const spentFfh = actions.some((a) => a.type === 'USE_ABILITY' && a.abilitySlug === 'ffh');
    check('R3: sorcerer holds ffh when only 1 enemy is in the blast', !spentFfh, JSON.stringify(actions));
}
...
// game if it scores (14 unblockable + kill potential on a wounded target).
{
    ...
    const actions = brain.selectActions(state, 'p1', map);
    const spentFfh = actions.some((a) => a.type === 'USE_ABILITY' && a.abilitySlug === 'ffh');
    check('R3: ffh gate lifts vs the last enemy (kills the 12-HP fighter)', spentFfh, JSON.stringify(actions));
}
// ===========================================================================
// V4 regression tests (sim-discovered bugs)
// ===========================================================================
const aiBrain_2 = require("./aiBrain");
const defaultData_2 = require("./defaultData");
...
// Old gate: <2 enemies hit while 2+ alive -> hold. New: a kill lifts it.
{
    ...
    check('V4 Bug 2: lethal single-target ffh bypasses the cluster gate', ffh, JSON.stringify(actions));
}
// --- Test 26 (V4 Bug 1): assassinate fires via move-to-reach mid-game ---
{
    ...
    check('V4 Bug 1: assassinate fires on 15-HP target 3 tiles away', used, JSON.stringify(actions));
}
...
{
    const raw = JSON.parse(JSON.stringify(defaultData_2.DEFAULT_ABILITIES));
    const assn = raw.find((a) => a.slug === 'assassinate');
    if (!assn)
        throw new Error('no assassinate def');
    const eff = assn.effects[0];
    eff['health_threshold'] = eff['healthThreshold'];
    delete eff['healthThreshold'];
    const fixedMap = new Map((0, aiBrain_2.normalizeAbilityDefinitions)(raw).map((a) => [a.slug, a]));
    const rogue = mkUnit('rogue', 'p1', { x: 3, y: 3 });
    ...
    check('V4: normalized snake_case def does NOT nuke a full-HP target', !wasted, JSON.stringify(actions));
...
// --- Test 33: immovable passive — Fear has no push value vs it ---
{
    ...
    const actions = brain.selectActions(state, 'p1', map);
    check('Fear skipped vs an immovable target (uses Demon Blast instead)', !actions.some((a) => a.type === 'USE_ABILITY' && a.abilitySlug === 'fear'), JSON.stringify(actions));
}
// --- Test 34: Round 1 — frozen units are never offered as the commit ---
{
    ...
    check('round 1: frozen(1) unit not committed (engine rejects pre-tick)', actsWith !== undefined && 'unitInstanceId' in actsWith && actsWith.unitInstanceId === healthy.instanceId, JSON.stringify(actions));
}
...
// --- Test 36: Ward — cleric shields a badly-threatened ally ---
{
    ...
    check('cleric takes SOME beneficial action toward the threatened ally', actions.some((a) => a.type === 'USE_ABILITY'));
}
...
{
    const scoreOf = (units, casterId) => {
        const state = mkState(units, casterId);
        const text = (0, aiBrain_3.explainTurn)(state, casterId, 'p1', map);
        const m = text.match(/roar @\([^)]*\) score=([-\d.]+)/);
        if (!m)
            throw new Error(`roar candidate not found in explainTurn output:\n${text}`);
    ...
    check('Roar score is unchanged by an ally sitting inside the blast radius (excludeAllies works)', Math.abs(scoreWithAlly - scoreWithoutAlly) < 0.001, `withoutAlly=${scoreWithoutAlly} withAlly=${scoreWithAlly}`);
}
...
// --- Test 40: Rescue — fighter pulls a badly threatened ranged ally to safety ---
{
    ...
    check('fighter takes SOME action when a ranged ally is double-teamed', actions.length > 0);
}
// --- Test 41: end-to-end vs the REAL engine with the DEFAULT roster (v7 regression) ---
{
    const r = (0, simHarness_1.runSim)(['barbarian', 'fighter', 'ranger', 'cleric'], ['rogue', 'rogue', 'sorcerer', 'sorcerer'], { games: 40 });
    check('real-engine run (default roster): zero validation errors over 40 games', r.totalValidationErrors === 0, `errors=${r.totalValidationErrors} sample=${r.sampleErrors[0] ?? ''}`);
    check('real-engine run (default roster): games resolve (avg turns sane)', r.avgTurns > 5 && r.avgTurns < 150, `avgTurns=${r.avgTurns}`);
}
...
// excludeAllies) in actual play — the strongest possible integration check.
{
    ...
    check('real-engine run (new specials + customization): games resolve (avg turns sane)', r.avgTurns > 5 && r.avgTurns < 150, `avgTurns=${r.avgTurns}`);
...
if (failures > 0)
    throw new Error(`${failures} smoke test failure(s)`);

...
function duel(comp, slot, passiveA, passiveB, games) {
    ...
    for (let g = 0; g < games; g++) {
        ...
        cA[slot] = { ...cA[slot], passiveSlug: passiveA };
        const cB = defaultLoadout(comp);
        cB[slot] = { ...cB[slot], passiveSlug: passiveB };
        const r = (0, simHarness_js_1.runMatch)(comp, comp, abilityMap, b1, b2, {
            p1Id: P1, p2Id: P2,
...
if (isMain) {
    ...
    for (const [compName, comp] of Object.entries(COMPS)) {
        for (let slot = 0; slot < comp.length; slot++) {
            ...
            const options = defaultData_js_1.DEFAULT_UNITS[slug].passiveOptions.map((p) => p.slug);
            ...
            for (let i = 0; i < options.length; i++) {
                for (let j = i + 1; j < options.length; j++) {
                    ...
                    console.log(`  ${options[i].padEnd(12)} vs ${options[j].padEnd(12)} ` +
...
//# sourceMappingURL=passiveArenaSim.js.map
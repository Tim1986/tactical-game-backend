...
function baselineOf(cell, party, slugs, games) {
    return (0, campaignSim_js_1.simEncounterCell)(cell.campaign, cell.encounter, cell.difficulty, party, slugs, {
...
for (const cell of allCells()) {
    for (const [party, slugs] of Object.entries(campaignSim_js_1.REPRESENTATIVE_PARTIES)) {
        const baseline = baselineOf(cell, party, slugs, pilotGames);
        const ok = baseline >= BAND_LO && baseline <= BAND_HI;
        if (ok)
            usable.push({ cell, party, slugs, baseline });
...
for (const u of usable) {
    ...
    for (const g of GIFTS) {
        const wr = (0, campaignSim_js_1.simEncounterCell)(u.cell.campaign, u.cell.encounter, u.cell.difficulty, u.party, u.slugs, {
...
for (const r of rows) {
    byParty[r.party] ??= {};
    for (const g of GIFTS)
        (byParty[r.party][g] ??= []).push(r.deltas[g]);
}
...
function encOf(cell) {
    return index_js_1.CAMPAIGNS[cell.campaign]?.encounters[cell.encounter];
}
function isPositional(cell) {
...
function shapeOf(cell) {
    ...
    if (win.some((w) => w.kind === 'ally_at_tiles'))
        return 'escort';
    if (win.some((w) => w.kind === 'units_at_tiles' && w.simultaneous))
        return 'hold';
    if (win.some((w) => w.kind === 'units_at_tiles'))
        return 'escape';
    if (win.some((w) => w.kind === 'round_reached'))
        return 'survive';
    if (e.rooms?.length)
        return 'rooms';
    if (loss.some((l) => l.kind === 'round_reached'))
        return 'race';
    if (e.waves?.length)
        return 'siege';
    if (win.some((w) => w.kind === 'units_dead'))
        return 'boss';
    if (e.terrain?.hazards?.length)
        return 'hazard';
    return 'kill-all';
}
...
//# sourceMappingURL=giftHarness.js.map
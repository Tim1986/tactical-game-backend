...
const ALL_CLASSES = Object.keys(defaultData_js_1.DEFAULT_UNITS);
const pct = (x) => (x * 100).toFixed(0).padStart(3) + '%';
const hit = (ac) => Math.min(1, Math.max(0, 1 - (ac - 6) * 0.05));
// Snapshot pristine stats once so sweep iterations don't compound.
const BASE_AC = Object.fromEntries(ALL_CLASSES.map((c) => [c, defaultData_js_1.DEFAULT_UNITS[c].armorClass]));
const BASE_HP = Object.fromEntries(ALL_CLASSES.map((c) => [c, defaultData_js_1.DEFAULT_UNITS[c].maxHealth]));
const PRESETS = {
    // ═══ POST-SHIP PRESETS ═══════════════════════════════════════════════════
...
const bestCusts = (slugs) => slugs.map((s2) => BEST_LOADOUT[s2]);
function applyDelta(delta) {
    for (const c of ALL_CLASSES) {
        ...
        defaultData_js_1.DEFAULT_UNITS[c].armorClass = Math.max(7, BASE_AC[c] + delta);
        defaultData_js_1.DEFAULT_UNITS[c].maxHealth = BASE_HP[c];
    }
}
const BASE_DMG = Object.fromEntries(defaultData_js_1.DEFAULT_ABILITIES.map((a) => [
    a.slug,
    a.effects.filter((e) => e.type === 'damage').map((e) => e.value ?? 0),
]));
const BASE_RANGE = Object.fromEntries(defaultData_js_1.DEFAULT_ABILITIES.map((a) => [a.slug, a.range]));
const BASE_SHAPE = Object.fromEntries(defaultData_js_1.DEFAULT_ABILITIES.map((a) => [a.slug, a.areaShape]));
const BASE_RADIUS = Object.fromEntries(defaultData_js_1.DEFAULT_ABILITIES.map((a) => [a.slug, a.areaRadius]));
const BASE_XA = Object.fromEntries(defaultData_js_1.DEFAULT_ABILITIES.map((a) => [a.slug, a.excludeAllies ?? false]));
const BASE_HEAL = Object.fromEntries(defaultData_js_1.DEFAULT_ABILITIES.map((a) => [
    a.slug,
    a.effects.filter((e) => e.type === 'heal').map((e) => e.value ?? 0),
]));
const BASE_SDUR = Object.fromEntries(defaultData_js_1.DEFAULT_ABILITIES.map((a) => [
    a.slug,
    a.effects.filter((e) => e.type === 'apply_status').map((e) => e.durationTurns ?? 0),
]));
const BASE_LS = Object.fromEntries(defaultData_js_1.DEFAULT_ABILITIES.map((a) => [
    a.slug,
    a.effects.filter((e) => e.type === 'lifesteal').map((e) => ({ v: e.value ?? 0, h: e.healValue ?? 0 })),
]));
const BASE_PASSIVES = Object.fromEntries(ALL_CLASSES.map((c) => [c, defaultData_js_1.DEFAULT_UNITS[c].passiveOptions.map((po) => ({ ...po }))]));
const BASE_TH = Object.fromEntries(defaultData_js_1.DEFAULT_ABILITIES.map((a) => [
    a.slug,
    a.effects.filter((e) => e.healthThreshold != null).map((e) => e.healthThreshold ?? 0),
]));
const BASE_SELFSTATUS = Object.fromEntries(defaultData_js_1.DEFAULT_ABILITIES.map((a) => [a.slug, a.selfStatus?.durationTurns ?? null]));
// Deep clone of every ability's original effects, so replaceEffects can be
// reverted between presets in one process.
const BASE_EFFECTS = Object.fromEntries(defaultData_js_1.DEFAULT_ABILITIES.map((a) => [a.slug, JSON.parse(JSON.stringify(a.effects))]));
const BASE_PP = Object.fromEntries(defaultData_js_1.DEFAULT_ABILITIES.map((a) => [
    a.slug,
    a.effects.filter((e) => e.type === 'push' || e.type === 'pull').map((e) => e.distance ?? 0),
]));
function applyPreset(p) {
    ...
    for (const k of Object.keys(abilityExecutor_js_1.OPPORTUNIST_BONUS_BY_CLASS))
        delete abilityExecutor_js_1.OPPORTUNIST_BONUS_BY_CLASS[k];
    for (const [c, v] of Object.entries(p.oppBonus ?? {}))
        abilityExecutor_js_1.OPPORTUNIST_BONUS_BY_CLASS[c] = v;
    for (const k of Object.keys(abilityExecutor_js_1.VENGEFUL_BONUS_BY_CLASS))
        delete abilityExecutor_js_1.VENGEFUL_BONUS_BY_CLASS[k];
    for (const [c, v] of Object.entries(p.vengBonus ?? {}))
        abilityExecutor_js_1.VENGEFUL_BONUS_BY_CLASS[c] = v;
    for (const c of ALL_CLASSES) {
        defaultData_js_1.DEFAULT_UNITS[c].armorClass = Math.max(7, BASE_AC[c] + (p.ac[c] ?? 0));
        defaultData_js_1.DEFAULT_UNITS[c].maxHealth = BASE_HP[c] + (p.hp[c] ?? 0);
    }
    for (const c of ALL_CLASSES) {
        const taxes = p.passiveHp?.[c];
        defaultData_js_1.DEFAULT_UNITS[c].passiveOptions = BASE_PASSIVES[c].map((po) => {
            ...
            return { ...po, stat: 'maxHealth', value: cur + d };
        });
    }
    for (const a of defaultData_js_1.DEFAULT_ABILITIES) {
        ...
        const srcEffects = (p.replaceEffects?.[a.slug] ?? BASE_EFFECTS[a.slug]);
        a.effects = JSON.parse(JSON.stringify(srcEffects));
        const localDmg = srcEffects.filter((e) => e.type === 'damage').map((e) => e.value ?? 0);
        const localHeal = srcEffects.filter((e) => e.type === 'heal').map((e) => e.value ?? 0);
        const localDur = srcEffects.filter((e) => e.type === 'apply_status').map((e) => e.durationTurns ?? 0);
        const localPP = srcEffects.filter((e) => e.type === 'push' || e.type === 'pull').map((e) => e.distance ?? 0);
        const selfBase = BASE_SELFSTATUS[a.slug];
        const dSelf = p.selfStatusDur?.[a.slug] ?? 0;
        if (selfBase != null) {
            a.selfStatus.durationTurns = selfBase + dSelf;
        }
        a.excludeAllies = p.excludeAllies?.[a.slug] ?? BASE_XA[a.slug];
        a.areaShape = p.areaShape?.[a.slug] ?? BASE_SHAPE[a.slug];
        a.areaRadius = BASE_RADIUS[a.slug] + (p.areaRadius?.[a.slug] ?? 0);
        const dTh = p.threshold?.[a.slug] ?? 0;
        let ti = 0;
        for (const e of a.effects) {
            if (e.healthThreshold != null) {
                e.healthThreshold = BASE_TH[a.slug][ti] + dTh;
                ti++;
            }
        }
        ...
        a.range = BASE_RANGE[a.slug] + (p.range?.[a.slug] ?? 0);
        const dPP = p.pullDist?.[a.slug] ?? 0;
        let i = 0, h = 0, d = 0, pp = 0;
        for (const e of a.effects) {
            if (e.type === 'damage') {
                e.value = localDmg[i] + dDmg;
                i++;
            }
            if (e.type === 'heal') {
                e.value = localHeal[h] + dHeal;
                h++;
            }
            if (e.type === 'apply_status') {
                e.durationTurns = localDur[d] + dDur;
                d++;
            }
            if (e.type === 'push' || e.type === 'pull') {
                e.distance = localPP[pp] + dPP;
                pp++;
            }
        }
...
function header(label) {
    ...
    for (const c of ALL_CLASSES) {
        const ac = defaultData_js_1.DEFAULT_UNITS[c].armorClass;
        const hp = defaultData_js_1.DEFAULT_UNITS[c].maxHealth;
        const ehp = hp / hit(ac);
        // Show any passive stat tax actually in force (Undying's HP cost).
        const taxed = defaultData_js_1.DEFAULT_UNITS[c].passiveOptions
...
function stageA(delta, games) {
    ...
    for (let i = 0; i < ALL_CLASSES.length; i++) {
        for (let j = i + 1; j < ALL_CLASSES.length; j++) {
            ...
            const r = (0, simHarness_js_1.runSim)(t1, t2, {
...
function stageB(delta, games) {
    ...
    for (let i = 0; i < COMPS.length; i++) {
        for (let j = i + 1; j < COMPS.length; j++) {
            ...
            const r = (0, simHarness_js_1.runSim)(pa.slugs, pb.slugs, {
...
function stageMarginals(duelGames, refGames) {
    ...
    for (const c of ALL_CLASSES) {
        const d = (0, loadoutMatrix_js_1.runDuelMatrix)(c, duelGames, () => { });
        for (const s of d.scores) {
            add(sd, `${c}/${s.loadout.specialSlug}`, s.wins, s.games);
            add(pd, `${c}/${s.loadout.passiveSlug ?? 'none'}`, s.wins, s.games);
        }
        const r = (0, loadoutMatrix_js_1.runReferenceMatrix)(c, refGames, () => { });
    ...
    const wr = (m) => (m && m.g > 0 ? m.w / m.g : NaN);
 ...
 */
function stagePairComps(games) {
    console.log(`\n──── Stage E: all pair-comps × full loadout grid (${games} games/cell) ────`);
    const L = (specialSlug, passiveSlug) => ({ specialSlug, passiveSlug });
    ...
    const FABLE_REFS = fableTeams_js_1.FABLE_TEAMS.map((t) => [t.name, [...t.slugs], (0, fableTeams_js_1.fableCustomizations)(t)]);
    const REFS = USE_FABLE_REFS ? FABLE_REFS : CLASSIC_REFS;
    ...
    if (partOwner)
        console.log(`  PART ${part}/${PART_CLASSES.length - 1}: pairs owned by "${partOwner}"`);
    for (let i = 0; i < ALL_CLASSES.length; i++) {
        for (let j = i + 1; j < ALL_CLASSES.length; j++) {
            const X = ALL_CLASSES[i], Y = ALL_CLASSES[j];
            if (onlyPair && !(onlyPair.includes(X) && onlyPair.includes(Y)))
                continue;
            ...
            if (focusClass && X !== focusClass && Y !== focusClass)
                continue;
            if (partOwner && (X < Y ? X : Y) !== partOwner)
                continue;
            const pair = `${X}²/${Y}²`;
            pairAgg[pair] = { w: 0, g: 0 };
            const lxs = (0, loadoutMatrix_js_1.loadoutsFor)(X), lys = (0, loadoutMatrix_js_1.loadoutsFor)(Y);
            for (const lx of lxs) {
                for (const ly of lys) {
                    const custs = [
                    ...
                    for (let k = 0; k < REFS.length; k++) {
                        const r = (0, simHarness_js_1.runSim)([X, X, Y, Y], REFS[k][1], {
                            games,
                        ...
                        for (const e of r.sampleErrors)
                            distinctErrors.add(`[vs ${REFS[k][0]}] ${e}`);
    ...
    if (distinctErrors.size > 0) {
        ...
        for (const e of [...distinctErrors].slice(0, 20))
            console.log(`    ${e}`);
    }
    ...
    if (csvPath) {
        ...
        (0, node_fs_1.writeFileSync)(csvPath, header + '\n' + csvRows.join('\n') + '\n');
...
function dumpAbilities(slugs) {
    console.log('\n  ABILITY DUMP (post-preset):');
    for (const slug of slugs) {
        const a = defaultData_js_1.DEFAULT_ABILITIES.find((x) => x.slug === slug);
        if (!a) {
            console.log(`    ${slug}: NOT FOUND`);
            continue;
        }
        const self = a.selfStatus ? ` self=${JSON.stringify(a.selfStatus)}` : '';
        const xa = a.excludeAllies ? ' EXCLUDE_ALLIES' : ' hits-allies';
        const shp = a.areaRadius ? ` shape=${a.areaShape ?? 'chebyshev'} r=${a.areaRadius}` : '';
...
if (presetName) {
    ...
    if (dump)
        dumpAbilities(dump.split(','));
    if (STAGE === 'a' || STAGE === 'all')
        stageA(1, GAMES);
    if (STAGE === 'b' || STAGE === 'all')
        stageB(1, GAMES);
    if (STAGE === 'c' || args.includes('--marginals'))
        stageMarginals(16, 40);
    if (STAGE === 'e' || args.includes('--paircomps'))
        stagePairComps(Number(flag('cellgames') ?? 40));
}
else {
...
//# sourceMappingURL=acExperiment.js.map
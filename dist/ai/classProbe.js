...
function parseDeltas(s) {
    if (!s)
        return {};
    const out = {};
    for (const part of s.split(',')) {
        const [k, v] = part.split(':');
        if (!loadoutMatrix_js_1.ALL_CLASSES.includes(k))
            throw new Error(`unknown class "${k}"`);
        out[k] = Number(v);
    }
    return out;
}
...
function parseAbilityDeltas(str) {
    if (!str)
        return {};
    const out = {};
...
for (const c of loadoutMatrix_js_1.ALL_CLASSES) {
    before[c] = { hp: defaultData_js_1.DEFAULT_UNITS[c].maxHealth, ac: defaultData_js_1.DEFAULT_UNITS[c].armorClass };
    defaultData_js_1.DEFAULT_UNITS[c].maxHealth += dHp[c] ?? 0;
    defaultData_js_1.DEFAULT_UNITS[c].armorClass = Math.max(7, defaultData_js_1.DEFAULT_UNITS[c].armorClass + (dAc[c] ?? 0));
}
...
for (const a of defaultData_js_1.DEFAULT_ABILITIES) {
    if (dRange[a.slug] != null) {
        a.range += dRange[a.slug];
        abilityChanges.push(`${a.slug} range +${dRange[a.slug]}`);
    }
    for (const e of a.effects) {
        if (dDmg[a.slug] != null && e.type === 'damage') {
            e.value += dDmg[a.slug];
            abilityChanges.push(`${a.slug} dmg +${dDmg[a.slug]}`);
        }
        if (dDur[a.slug] != null && e.type === 'apply_status') {
            e.durationTurns += dDur[a.slug];
            abilityChanges.push(`${a.slug} status +${dDur[a.slug]}t`);
        }
    }
}
/** Balanced 3-of-9 sample: the diagonal of the specials x passives square. */
function sampleLoadouts(cls) {
    const all = (0, loadoutMatrix_js_1.loadoutsFor)(cls);
    if (SAMPLE === 'full' || all.length < 9)
        return all;
    return [all[0], all[4], all[8]];
}
const REFS = fableTeams_js_1.FABLE_TEAMS.map((t) => ({
    name: t.name, slugs: [...t.slugs], custs: (0, fableTeams_js_1.fableCustomizations)(t),
}));
const cells = [];
const t0 = Date.now();
let errors = 0;
for (let i = 0; i < loadoutMatrix_js_1.ALL_CLASSES.length; i++) {
    for (let j = i + 1; j < loadoutMatrix_js_1.ALL_CLASSES.length; j++) {
        const X = loadoutMatrix_js_1.ALL_CLASSES[i], Y = loadoutMatrix_js_1.ALL_CLASSES[j];
        ...
        for (const lx of sampleLoadouts(X)) {
            for (const ly of sampleLoadouts(Y)) {
                ...
                for (let k = 0; k < REFS.length; k++) {
                    const r = (0, simHarness_js_1.runSim)([X, X, Y, Y], REFS[k].slugs, {
                        games: GAMES,
...
for (const cell of cells)
    for (const c of cell.pair.split('/'))
        chassis[c].push(cell.wr);
const ranked = loadoutMatrix_js_1.ALL_CLASSES.map((c) => ({ c, m: mean(chassis[c]) })).sort((a, b) => b.m - a.m);
...
const countTop = (k) => {
    ...
    for (const c of loadoutMatrix_js_1.ALL_CLASSES)
        t[c] = 0;
    for (const cell of sorted.slice(0, k))
        for (const c of cell.pair.split('/'))
            t[c]++;
    return t;
};
...
const changed = loadoutMatrix_js_1.ALL_CLASSES.filter((c) => dHp[c] || dAc[c])
    .map((c) => `${c} ${before[c].hp}→${defaultData_js_1.DEFAULT_UNITS[c].maxHealth}hp ${before[c].ac}→${defaultData_js_1.DEFAULT_UNITS[c].armorClass}ac`);
console.log(`\n═══ classProbe: ${LABEL} ═══`);
...
if (changed.length)
    console.log(`chassis: ${changed.join('  |  ')}`);
if (abilityChanges.length)
    console.log(`abilities: ${[...new Set(abilityChanges)].join('  |  ')}`);
...
//# sourceMappingURL=classProbe.js.map
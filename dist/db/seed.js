...
const ABILITIES = gameData_js_1.ABILITY_DEFS;
const UNITS = gameData_js_1.UNIT_DEFS;
// =============================================================
// ACHIEVEMENTS
// condition types:
...
async function seed() {
    ...
    try {
        ...
        // Status effects
        logger_js_1.logger.info('Seeding status effects...');
        for (const se of STATUS_EFFECTS) {
            await client.query(`INSERT INTO status_effect_definitions
         ...
         ON CONFLICT (slug) DO UPDATE SET
           ...
           max_stacks   = EXCLUDED.max_stacks`, [se.slug, se.name, se.description, se.trigger,
                JSON.stringify(se.effect), se.is_stackable, se.max_stacks]);
        }
        logger_js_1.logger.info(`Seeded ${STATUS_EFFECTS.length} status effects`);
        // Abilities
        ...
        // Units
        logger_js_1.logger.info('Seeding units...');
        for (const unit of UNITS) {
            await client.query(`INSERT INTO unit_definitions
         ...
         ON CONFLICT (slug) DO UPDATE SET
           ...
           is_active       = EXCLUDED.is_active`, [unit.slug, unit.name, unit.max_health, unit.armor_class, unit.movement_range,
        ...
        logger_js_1.logger.info(`Seeded ${UNITS.length} units`);
        // Achievements
        logger_js_1.logger.info('Seeding achievements...');
        for (const ach of ACHIEVEMENTS) {
            await client.query(`INSERT INTO achievement_definitions (slug, name, description, icon_key, condition, sort_order)
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT (slug) DO UPDATE SET
           name        = EXCLUDED.name,
           description = EXCLUDED.description,
           ...
           sort_order  = EXCLUDED.sort_order`, [ach.slug, ach.name, ach.description, ach.icon_key,
        ...
        logger_js_1.logger.error({ err }, 'Seed failed — rolled back');
        throw err;
    }
    finally {
        client.release();
...
seed().catch((err) => {
    logger_js_1.logger.error({ err }, 'Seed runner failed');
    process.exit(1);
});

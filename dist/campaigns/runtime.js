...
exports.DEEP_GIFTS = exports.GIFT_ARMOR_BONUS = exports.GIFT_MOVEMENT_BONUS = exports.hasSecondSpecialChargeAtLevel = exports.MAX_CAMPAIGN_LEVEL = exports.PLAYER_HP_DELTA = exports.CAMPAIGN_HP_SCALE = void 0;
...
exports.assertEncounterSupported = assertEncounterSupported;
 ...
 */
exports.PLAYER_HP_DELTA = { 1: -8, 2: -4, 3: -4, 4: 0, 5: 0, 6: 0, 7: 0, 8: 0, 9: 0, 10: 0 };
...
const hasSecondSpecialChargeAtLevel = (level) => level >= exports.MAX_CAMPAIGN_LEVEL;
exports.hasSecondSpecialChargeAtLevel = hasSecondSpecialChargeAtLevel;
...
exports.GIFT_MOVEMENT_BONUS = 1;
exports.GIFT_ARMOR_BONUS = 3;
exports.DEEP_GIFTS = {
    ...
    damage: { name: 'Gift of Fangs', description: `+${abilityExecutor_js_1.GIFT_DAMAGE_BONUS} damage on every damaging effect.` },
    movement: { name: 'Gift of Stride', description: `+${exports.GIFT_MOVEMENT_BONUS} movement range.`, movementRange: exports.GIFT_MOVEMENT_BONUS },
    armor: { name: 'Gift of Stone', description: `+${exports.GIFT_ARMOR_BONUS} armor class.`, armorClass: exports.GIFT_ARMOR_BONUS },
};
/**
...
function buildCampaignPlayerInstance(def, ownerId, position, level, choice) {
    ...
    const gift = choice?.deepGiftSlug !== undefined
        ? exports.DEEP_GIFTS[choice.deepGiftSlug] : undefined;
    ...
    const extraCharges = (0, exports.hasSecondSpecialChargeAtLevel)(level) && specialSlug
...
const UNIMPLEMENTED = [];
function assertEncounterSupported(campaign, encounterId) {
    const enc = campaign.encounters[encounterId];
    if (!enc)
        return; // buildEncounterState raises the proper unknown-encounter error
    const used = UNIMPLEMENTED.filter((f) => f.used(campaign, enc));
    if (used.length > 0) {
        throw new Error(`Encounter "${encounterId}" uses grammar not implemented yet: `
...
function validateCampaignAbilities(campaign) {
    for (const [slug, def] of Object.entries(campaign.abilities ?? {})) {
        if (def.slug !== slug)
            throw new Error(`Campaign ability "${slug}": slug field must match its key (got "${def.slug}")`);
        if (!KNOWN_TARGETING.has(def.targetingType))
            throw new Error(`Campaign ability "${slug}": unknown targetingType "${def.targetingType}"`);
        if (!Array.isArray(def.effects) || def.effects.length === 0)
            throw new Error(`Campaign ability "${slug}": needs at least one effect`);
        for (const e of def.effects) {
            if (!KNOWN_EFFECT_TYPES.has(e.type))
                throw new Error(`Campaign ability "${slug}": effect type "${e.type}" is not implemented by the executor`);
        }
...
function validateGoalsAndBoons(campaign, enc, encounterId) {
    const goals = enc.goals ?? [];
    if (goals.length > 2)
        throw new Error(`Encounter ${encounterId}: at most 2 battle goals (got ${goals.length})`);
    const seen = new Set();
    for (const g of goals) {
        if (seen.has(g.slug))
            throw new Error(`Encounter ${encounterId}: duplicate goal slug "${g.slug}"`);
        seen.add(g.slug);
        if (g.check.kind === 'win_by_round' && g.check.round < 1)
            throw new Error(`Encounter ${encounterId}: goal "${g.slug}" round must be >= 1`);
    }
    for (const [key, b] of Object.entries(campaign.boons ?? {})) {
        if (b.slug !== key)
            throw new Error(`Boon "${key}": slug field must match its key (got "${b.slug}")`);
        const fx = b.effects;
        ...
        if (fx.unitMaxHp && !defaultData_js_1.DEFAULT_UNITS[fx.unitMaxHp.classSlug]) {
...
function applyBoons(partyUnits, boons) {
    for (const b of boons) {
        ...
        if (fx.partyMovement)
            for (const u of partyUnits)
                u.movementRange = Math.max(1, u.movementRange + fx.partyMovement);
        if (fx.partyArmorClass)
            for (const u of partyUnits)
                u.armorClass = Math.max(1, u.armorClass + fx.partyArmorClass);
        if (fx.unitMaxHp) {
            for (const u of partyUnits) {
                if (u.definitionSlug === fx.unitMaxHp.classSlug)
                    bumpHp(u, fx.unitMaxHp.amount);
            }
        }
        if (fx.startShielded) {
...
hpScaleOverride) {
    ...
    const runBoons = (boonKeys ?? []).map((k) => {
        ...
        if (!b)
            throw new Error(`Encounter ${encounterId}: run carries unknown boon "${k}"`);
        return b;
    });
    ...
    const abilityPool = new Set([
        ...defaultData_js_1.DEFAULT_ABILITIES.map((a) => a.slug),
        ...Object.keys(campaign.abilities ?? {}),
    ]);
    const checkKit = (slugs, who) => {
        for (const slug of slugs ?? []) {
            if (!abilityPool.has(slug))
                throw new Error(`Encounter ${encounterId}: ${who} references unknown ability "${slug}"`);
        }
    };
    for (const [key, enemy] of Object.entries(campaign.enemies))
        checkKit(enemy.abilities, `enemy "${key}"`);
    for (const [key, ally] of Object.entries(enc.allies ?? {}))
        checkKit(ally.abilities, `ally "${key}"`);
    // A4: `rooms` REPLACES the top-level board fields — rooms[0] is the opening
    // board and the encounter playerPlacement is its entry.
    if (enc.rooms && enc.rooms.length === 0)
        throw new Error(`Encounter ${encounterId}: rooms must not be empty`);
    const room0 = enc.rooms?.[0];
    const effTerrainSpec = room0 ? room0.terrain : enc.terrain;
    ...
    const hpScale = hpScaleOverride ?? enc.hpScaleOverride?.[difficulty] ?? exports.CAMPAIGN_HP_SCALE[difficulty];
    ...
    for (const b of terrainBlocked) {
        if (!(0, boardUtils_js_1.isInBounds)(b))
            throw new Error(`Encounter ${encounterId}: wall (${b.x},${b.y}) is out of bounds`);
    }
    for (const h of terrainHazards) {
        if (!(0, boardUtils_js_1.isInBounds)(h.pos))
            throw new Error(`Encounter ${encounterId}: hazard (${h.pos.x},${h.pos.y}) is out of bounds`);
    ...
    // side of the fight).
    const allyIdsByKey = new Map();
    const allyBehaviors = {};
    const allyUnits = Object.entries(enc.allies ?? {}).map(([key, ally]) => {
        const def = defaultData_js_1.DEFAULT_UNITS[ally.baseClass];
        if (!def)
            throw new Error(`Encounter ${encounterId}: ally "${key}" has unknown baseClass ${ally.baseClass}`);
        const pl = ally.placement;
        if (!(0, boardUtils_js_1.isInBounds)(pl))
            throw new Error(`Encounter ${encounterId}: ally "${key}" placement out of bounds`);
        if (terrainBlocked.some((b) => b.x === pl.x && b.y === pl.y))
            throw new Error(`Encounter ${encounterId}: ally "${key}" placed on a wall`);
        if (terrainHazards.some((h) => h.pos.x === pl.x && h.pos.y === pl.y))
            throw new Error(`Encounter ${encounterId}: ally "${key}" placed on a hazard`);
        if (ally.behavior.mode === 'route') {
            if (!ally.behavior.waypoints.length)
                throw new Error(`Encounter ${encounterId}: ally "${key}" route needs waypoints`);
            for (const w of ally.behavior.waypoints) {
                if (!(0, boardUtils_js_1.isInBounds)(w) || terrainBlocked.some((b) => b.x === w.x && b.y === w.y)) {
    ...
    const wallOf = (t) => t?.blocked ?? [];
    const buildSpawnGroup = (keys, noSpec) => keys.map((key) => {
        const enemy = campaign.enemies[key];
        if (!enemy)
            throw new Error(`Encounter ${encounterId}: unknown enemy key "${key}" in wave/room`);
        const inst = buildCampaignEnemyInstance(enemy, enemyOwnerId, { x: 0, y: 0 }, difficulty, hpScale, noSpec);
        unitNames[inst.instanceId] = enemy.name;
        enemyIdsByKey.set(key, [...(enemyIdsByKey.get(key) ?? []), inst.instanceId]);
        return inst;
    });
    const resolveWaves = (waves, roomTerrain, noSpec, where) => (waves ?? []).map((w, wi) => {
        if (w.enemies.length === 0)
            throw new Error(`Encounter ${encounterId}: ${where} wave ${wi} has no enemies`);
        for (const pl of w.placement) {
            if (!(0, boardUtils_js_1.isInBounds)(pl))
                throw new Error(`Encounter ${encounterId}: ${where} wave ${wi} spawn (${pl.x},${pl.y}) is out of bounds`);
        ...
        if (w.trigger.on === 'door') {
            ...
            if (!(0, boardUtils_js_1.isInBounds)(t) || wallOf(roomTerrain).some((b) => b.x === t.x && b.y === t.y)) {
    ...
    if (room0) {
        const later = enc.rooms.slice(1).map((r, i) => {
            const idx = i + 1;
            const rWalls = wallOf(r.terrain);
            const rHaz = r.terrain?.hazards ?? [];
            for (const b of rWalls)
                if (!(0, boardUtils_js_1.isInBounds)(b))
                    throw new Error(`Encounter ${encounterId}: room ${idx} wall out of bounds`);
            for (const h of rHaz) {
                if (!(0, boardUtils_js_1.isInBounds)(h.pos))
                    throw new Error(`Encounter ${encounterId}: room ${idx} hazard out of bounds`);
                if (rWalls.some((b) => b.x === h.pos.x && b.y === h.pos.y))
                    throw new Error(`Encounter ${encounterId}: room ${idx} hazard on a wall`);
            }
            if (!r.entryTiles?.length)
                throw new Error(`Encounter ${encounterId}: room ${idx} needs entryTiles`);
            for (const t of [...r.entryTiles, ...r.enemyPlacement, ...(r.exitDoors ?? [])]) {
                if (!(0, boardUtils_js_1.isInBounds)(t))
                    throw new Error(`Encounter ${encounterId}: room ${idx} tile (${t.x},${t.y}) out of bounds`);
                if (rWalls.some((b) => b.x === t.x && b.y === t.y))
                    throw new Error(`Encounter ${encounterId}: room ${idx} tile (${t.x},${t.y}) is on a wall`);
            }
            for (const t of r.entryTiles) {
                if (rHaz.some((h) => h.pos.x === t.x && h.pos.y === t.y))
                    throw new Error(`Encounter ${encounterId}: room ${idx} entry tile (${t.x},${t.y}) is on a hazard`);
            }
            const isLast = idx === enc.rooms.length - 1;
            if (!isLast && !(r.exitDoors?.length))
                throw new Error(`Encounter ${encounterId}: room ${idx} needs exitDoors (not the last room)`);
            return {
        ...
        if (!(room0.exitDoors?.length))
            throw new Error(`Encounter ${encounterId}: room 0 needs exitDoors`);
        for (const d of room0.exitDoors) {
            if (!(0, boardUtils_js_1.isInBounds)(d) || terrainBlocked.some((b) => b.x === d.x && b.y === d.y)) {
    ...
    if (enc.objective) {
        ...
        const checkTiles = (tiles, what) => {
            for (const t of tiles) {
                if (!(0, boardUtils_js_1.isInBounds)(t))
                    throw new Error(`Encounter ${encounterId}: ${what} tile (${t.x},${t.y}) is out of bounds`);
                if (terrainBlocked.some((b) => b.x === t.x && b.y === t.y)) {
                    throw new Error(`Encounter ${encounterId}: ${what} tile (${t.x},${t.y}) is on a wall`);
                }
            }
        };
        const win = spec.win.map((w) => {
            switch (w.kind) {
                case 'units_dead': {
                    const unitIds = w.enemyKeys.flatMap((k) => {
                        const ids = enemyIdsByKey.get(k);
                        if (!ids)
                            throw new Error(`Encounter ${encounterId}: objective names unknown enemy key "${k}"`);
                        return ids;
                    });
                ...
                case 'ally_at_tiles': {
                    ...
                    if (!id)
                        throw new Error(`Encounter ${encounterId}: objective names unknown ally "${w.allyKey}"`);
                    checkTiles(w.tiles, 'objective');
        ...
        const loss = (spec.loss ?? []).map((l) => {
            if (l.kind === 'ally_dead') {
                const id = allyIdsByKey.get(l.allyKey);
                if (!id)
                    throw new Error(`Encounter ${encounterId}: objective names unknown ally "${l.allyKey}"`);

...
function applyPull(ctx, target, effect) {
    ...
    if (hasPassive(target, 'immovable'))
        return;
    const destination = (0, boardUtils_js_1.calculatePullDestination)(target.position, ctx.caster.position, effect.distance);
    const finalPos = findLastFreePosition(target.position, destination, ctx.state.units, target.instanceId);
    target.position = finalPos;
...
function findLastFreePosition(start, end, units, movingUnitId) {

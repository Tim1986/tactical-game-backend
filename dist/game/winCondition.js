...
const ONGOING = { isOver: false, winnerId: null, loserId: null };
/**
...
function checkWinCondition(state, playerOneId, playerTwoId) {
    ...
    if (obj)
        return checkObjective(state, obj);
...
function livingOf(state, ownerId) {
    return state.units.filter((u) => u.isAlive && u.ownerPlayerId === ownerId);
}
const onAnyTile = (u, tiles) => tiles.some((t) => t.x === u.position.x && t.y === u.position.y);
function winSatisfied(state, obj, c) {
    switch (c.kind) {
        case 'all_enemies_dead':
            // A4: pending waves/rooms mean the fight is not over on a clear board.
            if ((0, encounterFlow_js_1.hasPendingContent)(state))
                return null;
...
function lossSatisfied(state, obj, c) {
    switch (c.kind) {
        case 'ally_dead': {
...
function checkObjective(state, obj) {
    ...
    if (!(0, encounterFlow_js_1.hasPendingContent)(state) && livingOf(state, obj.enemyId).length === 0) {
    ...
    for (const c of obj.loss) {
        ...
        if (reason)
            return { isOver: true, winnerId: obj.enemyId, loserId: obj.partyId, reason };
    }
    return ONGOING;
}

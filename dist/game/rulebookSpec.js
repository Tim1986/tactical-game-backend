...
exports.RULE_CHECKS = void 0;
exports.runRulebookChecks = runRulebookChecks;
...
const defOf = (slug) => defaultData_js_1.DEFAULT_UNITS[slug];
...
const P2 = 'player-two';
let _seq = 0;
function mkUnit(owner, x, y, over = {}) {
    return {
...
function mkLegacyState(units, active = P1) {
    return {
...
function mkInitiativeState(units, order, slot = 0) {
...
exports.RULE_CHECKS = [
    // ── BRD ────────────────────────────────────────────────────────────────────
    {
        ...
        run: () => {
            ...
            for (let x = 0; x < 8; x++)
                for (let y = 0; y < 8; y++)
                    if ((0, boardUtils_js_1.isInBounds)({ x, y }))
                        count++;
            assert(count === 60, `expected 60 playable tiles, got ${count}`);
            for (const c of [[0, 0], [0, 7], [7, 0], [7, 7]]) {
                assert(!(0, boardUtils_js_1.isInBounds)({ x: c[0], y: c[1] }), `corner (${c[0]},${c[1]}) must be out of bounds`);
                assert((0, boardUtils_js_1.isCorner)(c[0], c[1]) && (0, geometry_js_1.isCorner)(c[0], c[1]), 'boardUtils.isCorner and geometry.isCorner must agree');
            }
            assert(!(0, boardUtils_js_1.isInBounds)({ x: -1, y: 3 }) && !(0, boardUtils_js_1.isInBounds)({ x: 8, y: 3 }), 'off-board tiles out of bounds');
            // a corner is never reachable nor on a path
            const u = mkUnit(P1, 1, 0);
            assert(!at((0, geometry_js_1.reachableFrom)(u.position, u, [u], 4), 0, 0), 'removed corner must never be reachable');
        },
    },
    {
        ...
        run: () => {
            ...
            for (const dest of [ally.position, enemy.position]) {
                assertThrows(() => (0, turnProcessor_js_1.processTurn)(state, [{ type: 'MOVE', unitInstanceId: mover.instanceId, destination: dest }, { type: 'END_TURN' }], P1, P1, P2, new Map()), 'occupied', `move onto occupied tile (${dest.x},${dest.y}) must be rejected`);
            }
        },
    },
    // ── TRN ────────────────────────────────────────────────────────────────────
    {
        ...
        run: () => {
            ...
            const r = (0, turnProcessor_js_1.processTurn)(state, [{ type: 'MOVE', unitInstanceId: unit.instanceId, destination: unit.position }, { type: 'END_TURN' }], first, P1, P2, new Map());
    ...
    },
    {
        ...
        run: () => {
            const state = mkRound1State();
            assertThrows(() => (0, turnProcessor_js_1.processTurn)(state, [{ type: 'END_TURN' }], state.activePlayerId, P1, P2, new Map()), 'commit', 'bare END_TURN in round 1 must be rejected');
        },
    },
    {
        ...
        run: () => {
            ...
            for (let i = 0; i < 8; i++) {
                ...
                state = (0, turnProcessor_js_1.processTurn)(state, [{ type: 'MOVE', unitInstanceId: unit.instanceId, destination: unit.position }, { type: 'END_TURN' }], pid, P1, P2, new Map()).updatedState;
            }
            const init = state.initiative;
            assert(!init.isRound1 && init.order.length === 8, 'all 8 commits must end round 1');
            const ownerOf = (id) => state.units.find((u) => u.instanceId === id).ownerPlayerId;
            for (let i = 0; i < 8; i++) {
    ...
    },
    {
        ...
        run: () => {
            ...
            const r = (0, turnProcessor_js_1.processTurn)(state, [
    ...
    },
    {
        ...
        run: () => {
            ...
            const r = (0, turnProcessor_js_1.processTurn)(state, [{ type: 'END_TURN' }], P1, P1, P2, new Map());
    ...
    },
    {
        ...
        run: () => {
            ...
            const ok = (0, turnProcessor_js_1.processTurn)(mkLegacyState([a, b]), [
            ...
            assertThrows(() => (0, turnProcessor_js_1.processTurn)(mkLegacyState([mkUnit(P1, 1, 1), b]), [
            ...
            const a2 = mkUnit(P1, 1, 1);
            const b2 = mkUnit(P2, 7, 6);
            const late = mkInitiativeState([a2, b2], [a2.instanceId, b2.instanceId], 0);
            late.roundNumber = 11;
            late.turnNumber = 81;
            const lateOk = (0, turnProcessor_js_1.processTurn)(late, [
            ...
            const wall1 = mkUnit(P2, 0, 4);
            const wall2 = mkUnit(P2, 1, 3);
            const wall3 = mkUnit(P2, 1, 4);
            assertThrows(() => (0, turnProcessor_js_1.processTurn)(mkLegacyState([walled, wall1, wall2, wall3]), [
    ...
    // ── MOV ────────────────────────────────────────────────────────────────────
    {
        ...
        run: () => {
            ...
            const tiles = (0, geometry_js_1.reachableFrom)(u.position, u, [u], 1);
            ...
            const two = (0, geometry_js_1.reachableFrom)(u.position, u, [u], 2);
    ...
    },
    {
        ...
        run: () => {
            ...
            const tiles = (0, geometry_js_1.reachableFrom)(u.position, u, [u, ally], 2);
    ...
    },
    {
        ...
        run: () => {
            ...
            const tiles = (0, geometry_js_1.reachableFrom)(u.position, u, [u, enemy], 2);
            ...
            const w1 = mkUnit(P2, 0, 4);
            const w2 = mkUnit(P2, 1, 4);
            const w3 = mkUnit(P2, 1, 3);
            const path = (0, geometry_js_1.findPath)(walled.position, { x: 0, y: 6 }, walled, [walled, w1, w2, w3]);
            assert(path === null || path.every((p) => !(p.x === 0 && p.y === 4) && !(p.x === 1 && p.y === 4) && !(p.x === 1 && p.y === 3)), 'findPath must never route through an enemy tile');
            const open = (0, geometry_js_1.findPath)(u.position, { x: 2, y: 3 }, u, [u, enemy]);
    ...
    },
    {
        ...
        run: () => {
            ...
            let u = rooted();
            assertThrows(() => (0, turnProcessor_js_1.processTurn)(mkLegacyState([u, b]), [
            ...
            u = rooted();
            const hold = (0, turnProcessor_js_1.processTurn)(mkLegacyState([u, b]), [
            ...
            u = rooted();
            const act = (0, turnProcessor_js_1.processTurn)(mkLegacyState([u, b]), [
    ...
    // ── ABL ────────────────────────────────────────────────────────────────────
    {
        ...
        run: () => {
            ...
            const r = (0, turnProcessor_js_1.processTurn)(mkLegacyState([u2, mkUnit(P2, 2, 1)]), [
            ...
            assertThrows(() => (0, turnProcessor_js_1.processTurn)(mkLegacyState([onCd, mkUnit(P2, 2, 1)]), [
            ...
            (0, abilityExecutor_js_2.tickUnitCooldowns)(after);
            const cdAfterTick = after.cooldowns['test_hit'];
    ...
    },
    {
        ...
        run: () => {
            ...
            assertThrows(() => (0, turnProcessor_js_1.processTurn)(mkLegacyState([u, diag]), [
            ...
            const u2 = mkUnit(P1, 1, 1);
            const ok = (0, turnProcessor_js_1.processTurn)(mkLegacyState([u2, ortho]), [
    ...
    },
    {
        ...
        run: () => {
            const map = (ab) => new Map([[ab.slug, ab]]);
            const shooter = mkUnit(P1, 1, 1);
            const blocker = mkUnit(P2, 3, 1);
            const target = mkUnit(P2, 5, 1);
            assertThrows(() => (0, turnProcessor_js_1.processTurn)(mkLegacyState([shooter, blocker, target]), [
            ...
            const okNonAligned = (0, turnProcessor_js_1.processTurn)(mkLegacyState([s2, off, t2]), [
            ...
            const okPush = (0, turnProcessor_js_1.processTurn)(mkLegacyState([s3u, b3, t3]), [
    ...
    },
    {
        ...
        run: () => {
            ...
            for (const def of defaultData_js_1.DEFAULT_ABILITIES.filter((a) => a.targetingType === 'aoe')) {
                ...
                for (const enemy of [near, diag]) {
                    ...
                    const predicted = (0, boardUtils_js_2.isInAoe)(center, pos, def.areaRadius, def.areaShape);
                    const actual = events.some((e) => e.targetUnitInstanceId === enemy.instanceId);
                    assert(predicted === actual, `${def.slug}: engine hit=${actual} but isInAoe predicts ${predicted} for enemy at (${pos.x},${pos.y})`);
                }
            }
        },
    },
    // ── STA ────────────────────────────────────────────────────────────────────
    {
        ...
        run: () => {
            ...
            const ev = [];
            (0, abilityExecutor_js_2.applyStartOfTurnStatusDamage)(u, ev);
    ...
    },
    {
        ...
        run: () => {
            ...
            (0, abilityExecutor_js_2.applyStartOfTurnStatusDamage)(u, []);
            ...
            const ev = [];
            (0, abilityExecutor_js_2.applyStartOfTurnStatusDamage)(dying, ev);
    ...
    // ── PAS ────────────────────────────────────────────────────────────────────
    {
        ...
        run: () => {
            const base = (0, initialState_js_1.buildUnitInstance)(defOf('rogue'), P1, { x: 1, y: 1 });
            const swift = (0, initialState_js_1.buildUnitInstance)(defOf('rogue'), P1, { x: 1, y: 1 }, { specialSlug: 'assassinate', passiveSlug: 'swift' });
    ...
    },
    {
        ...
        run: () => {
            const mkThorns = (x, y) => mkUnit(P2, x, y, { passives: ['thorns'] });
            ...
            const r = (0, turnProcessor_js_1.processTurn)(st, [
    ...
    },
    {
        ...
        run: () => {
            ...
            const tickEv = [];
            (0, abilityExecutor_js_2.applyStartOfTurnStatusDamage)(u, tickEv);
    ...
    // ── END ───────────────────────────────────────────────────────────────────
    {
        ...
        run: () => {
            ...
            const r2 = (0, turnProcessor_js_1.processTurn)(r.updatedState, [{ type: 'END_TURN' }], P2, P1, P2, new Map());
    ...
    },
    {
        ...
        run: () => {
            ...
            const a = mkUnit(P1, 1, 1, { currentHealth: 20 });
            const b = mkUnit(P2, 7, 1);
            const state = mkInitiativeState([a, b], [a.instanceId, b.instanceId], 0);
            ...
            const r = (0, turnProcessor_js_1.processTurn)(retreatState, [
            ...
            const r2 = (0, turnProcessor_js_1.processTurn)(advanceState, [
                { type: 'MOVE', unitInstanceId: a.instanceId, destination: { x: 2, y: 1 } },
                { type: 'END_TURN' },
            ], P1, P1, P2, new Map());
            ...
            const r3 = (0, turnProcessor_js_1.processTurn)(holdState, [{ type: 'END_TURN' }], P1, P1, P2, new Map());
    ...
    },
    {
        ...
        run: () => {
            ...
            const r = (0, turnProcessor_js_1.processTurn)(JSON.parse(JSON.stringify(state)), [
                { type: 'MOVE', unitInstanceId: a.instanceId, destination: { x: 1, y: 0 } },
                { type: 'END_TURN' },
            ], P1, P1, P2, new Map());
    ...
    },
    {
        ...
        run: () => {
            ...
            const dead = mkUnit(P2, 2, 1, { isAlive: false, currentHealth: 0 });
            const over = (0, winCondition_js_1.checkWinCondition)(mkLegacyState([alive, dead]), P1, P2);
            assert(over.isOver && over.winnerId === P1 && over.loserId === P2, 'wiping a team must end the match');
            const ongoing = (0, winCondition_js_1.checkWinCondition)(mkLegacyState([mkUnit(P1, 1, 1), mkUnit(P2, 2, 1)]), P1, P2);
            assert(!ongoing.isOver, 'match must continue while both sides have units');
        },
    },
];
// ─── Round-1 fixture (needs 4v4 with initiative) ─────────────────────────────
function mkRound1State() {
    const units = [
...
function runRulebookChecks() {
    const failures = [];
    for (const check of exports.RULE_CHECKS) {
        try {
            check.run();
        }
        catch (e) {
...
//# sourceMappingURL=rulebookSpec.js.map
...
exports.hasPendingContent = hasPendingContent;
exports.weaveIntoInitiative = weaveIntoInitiative;
exports.spawnWave = spawnWave;
exports.checkSpawnTriggers = checkSpawnTriggers;
exports.maybeRoomTransition = maybeRoomTransition;
...
function hasPendingContent(state) {
 ...
 */
function resolveSpawnTile(state, want) {
    const free = (p) => (0, boardUtils_js_1.isInBounds)(p)
        && !(0, geometry_js_1.isTerrainBlocked)(state.terrain, p)
 ...
 */
function weaveIntoInitiative(state, spawned, events) {
    const initiative = state.initiative;
    if (initiative.isRound1)
        return; // joins the round-1 commit pool naturally
    ...
    for (const unit of spawned) {
        ...
        for (let i = 0; i < order.length; i++) {
            if (!partyIds.has(order[i]))
                continue;
            const next = order[i + 1];
            if (next === undefined || partyIds.has(next)) {
                insertAt = i + 1;
                break;
            }
        }
        ...
        if (insertAt <= initiative.slot)
            initiative.slot += 1;
    }
    void events;
}
/** Spawn one wave's units onto the current board and weave them in. */
function spawnWave(state, wave, events) {
    const spawned = [];
    wave.units.forEach((unit, i) => {
        ...
        if (!tile)
            return; // board completely packed — drop the spawn (authored content should never hit this)
        unit.position = tile;
        if (wave.surprise)
            unit.skipFirstSlot = true;
 ...
 */
function checkSpawnTriggers(state, events, mover) {
    const ep = state.encounterProgress;
    if (!ep || ep.waves.length === 0)
        return;
    ...
    for (const w of ep.waves) {
        const fire = (w.trigger.on === 'round' && state.roundNumber >= w.trigger.round)
 ...
 */
function maybeRoomTransition(state, mover, events) {
    const ep = state.encounterProgress;
    if (!ep || ep.rooms.length === 0)
        return false;
    if (!ep.partyIds.includes(mover.instanceId))
        return false;
    const onDoor = ep.exitDoors.some((d) => d.x === mover.position.x && d.y === mover.position.y);
    if (!onDoor)
        return false;
    if (ep.doorMode === 'on_clear' && livingEnemies(state).length > 0)
        return false;
    ...
    state.initiative.order = state.initiative.order.filter((id, idx) => {
        if (!leftIds.has(id))
            return true;
        if (idx <= state.initiative.slot)
            state.initiative.slot -= 1;
        return false;
    });
    // New carve.
    ...
    living.forEach((u, i) => {
        ...
        if (tile) {
            ...
            (0, abilityExecutor_js_1.applyEntryHazard)(state, u, events); // authored entry tiles are validated hazard-free; displaced landings are not
        }
    });
...
//# sourceMappingURL=encounterFlow.js.map
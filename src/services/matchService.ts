...
function buildUnitInstance(def: UnitDefinition, ownerId: string, position: BoardPosition): UnitInstance {
  ...
  return {
    ...
    ...({ movementRange: def.movementRange } as object),
    ...({ abilities: def.abilities } as object),
    ...({ passives: def.passives } as object),
  } as UnitInstance;
}
...
async function loadTeamUnits(teamId: string): Promise<UnitDefinition[]> {
  ...
  const unitResult = await query<{ id: string; slug: string; name: string; max_health: number; movement_range: number; abilities: string[]; passives: string[]; unlock_level: number; asset_key: string; is_active: boolean; }>(
    'SELECT id, slug, name, max_health, movement_range, abilities, passives, unlock_level, asset_key, is_active FROM unit_definitions WHERE id = ANY($1)',
    [team.unit_ids]
  );
...
async function loadAbilityMap(client: import('pg').PoolClient): Promise<Map<string, AbilityDefinition>> {
  const result = await client.query<{ id: string; slug: string; name: string; description: string; targeting_type: string; range: number; area_radius: number; cooldown_turns: number; effects: unknown[]; }>(
    'SELECT id, slug, name, description, targeting_type, range, area_radius, cooldown_turns, effects FROM ability_definitions'
  );
  const map = new Map<string, AbilityDefinition>();
  for (const row of result.rows) {
    map.set(row.slug, { id: row.id, slug: row.slug, name: row.name, description: row.description, targetingType: row.targeting_type as AbilityDefinition['targetingType'], range: row.range, areaRadius: row.area_radius, cooldownTurns: row.cooldown_turns, effects: row.effects as AbilityDefinition['effects'] });
  }
  return map;
}
